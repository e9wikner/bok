...
def seed_test_company():
    ...
    try:
        ...
        cursor = db.execute(
            "SELECT id FROM fiscal_years WHERE start_date = '2026-01-01' AND end_date = '2026-12-31' LIMIT 1"
        )
        row = cursor.fetchone()
        if row:
    ...
    for account_code in sorted(balances.keys()):
        ...
        print(
            f"      {account_code} {account.name:30} | Debit: {debit:>12,.2f} | Credit: {credit:>12,.2f}"
        )
    ...
    print(
        f"      ✓ Invoice {inv1.invoice_number} created (ex VAT: {inv1.amount_ex_vat/100:,.0f} kr)"
    )

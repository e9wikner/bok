    ...
    def create_customer(
        ...
        if payment_terms_days < 0:
            raise ValidationError(
                "invalid_payment_terms", "Payment terms must be non-negative"
            )
        if org_number and CustomerRepository.find_by_org_number(org_number):
            raise ValidationError(
                "duplicate_customer", "Customer org number already exists"
            )
...
class ArticleService:
    def create_article(
        ...
        if not article_number.strip() or not name.strip():
            raise ValidationError(
                "invalid_article", "Article number and name are required"
            )
        ...
        if unit_price < 0:
            raise ValidationError(
                "invalid_unit_price", "Unit price must be non-negative"
            )

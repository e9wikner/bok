...
class SIE4Importer:
    ...
    def import_content(self, content: str, fiscal_year_id: Optional[str] = None) -> bool:
    ...
    def _import_account(self, account: SIEAccount) -> bool:
        ...
        resp = requests.get(
            f"{self.api_url}/api/v1/accounts/{account.code}",
        ...
        resp = requests.post(
            f"{self.api_url}/api/v1/accounts",
            headers=self.headers,
            json=account_data
        )
    ...
    def _import_voucher(self, voucher: SIEVoucher, fiscal_year_id: Optional[str] = None) -> bool:
        ...
        if resp.status_code == 201:
            ...
            self.errors.append(f"Failed to create voucher {voucher.series}{voucher.number}: {resp.text}")
            return False
    
    def _get_or_create_period(self, voucher_date: date, fiscal_year_id: Optional[str] = None) -> Optional[str]:
        ...
        if not fiscal_year_id:
            resp = requests.get(
                f"{self.api_url}/api/v1/fiscal-years",
                headers=self.headers
            )
            if resp.status_code == 200:

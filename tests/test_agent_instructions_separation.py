...
class TestSystemInstructions:
    ...
    def test_get_accounting_system_instructions(self):
        ...
        assert result["scope"] == "system"
        assert (
            "drift_och_atkomst" in result["content_markdown"].lower()
            or "miljö" in result["content_markdown"].lower()
        )
        assert (
            "bokföringsprocess" in result["content_markdown"].lower()
            or "verifikation" in result["content_markdown"].lower()
        )
        assert (
            "sie4" in result["content_markdown"].lower()
            or "import" in result["content_markdown"].lower()
        )
...
class TestAgentInstructionsAPI:
    ...
    @pytest.mark.asyncio
    async def test_get_accounting_instructions_structure(
        self, test_db, async_client, auth_headers
    ):
        """GET /accounting should return both system and company."""
        response = await async_client.get(
            "/api/v1/agent-instructions/accounting", headers=auth_headers
        )
    ...
    @pytest.mark.asyncio
    async def test_get_invoicing_instructions_structure(
        self, test_db, async_client, auth_headers
    ):
        """GET /invoicing should return both system and company."""
        response = await async_client.get(
            "/api/v1/agent-instructions/invoicing", headers=auth_headers
        )
    ...
    @pytest.mark.asyncio
    async def test_accounting_versions_endpoint(
        self, test_db, async_client, auth_headers
    ):
        """GET /accounting/versions should list company versions."""
        response = await async_client.get(
            "/api/v1/agent-instructions/accounting/versions", headers=auth_headers
        )
    ...
    async def test_system_only_endpoints(self, async_client, auth_headers):
        ...
        response = await async_client.get(
            "/api/v1/agent-instructions/system/accounting", headers=auth_headers
        )
        ...
        assert (
            "SIE4" in data["content_markdown"]
            or "import" in data["content_markdown"].lower()
        )
...
class TestBackwardCompatibility:
    ...
    @pytest.mark.asyncio
    async def test_legacy_accounting_endpoint(
        self, test_db, async_client, auth_headers
    ):
        """Legacy endpoint should still work."""
        response = await async_client.get(
            "/api/v1/agent-instructions/accounting/legacy", headers=auth_headers
        )

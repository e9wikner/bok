...
INK2R_ROWS: Tuple[INK2RRow, ...] = (
    ...
    INK2RRow(
        ...
        _ASSETS,
        ((1320, 1329), (1340, 1345), (1348, 1349)),
    ),
    INK2RRow(
        "2.11", "7234", "Lån till delägare eller närstående", _ASSETS, ((1360, 1369),)
    ),
    INK2RRow(
        "2.12",
        "7235",
    ...
    INK2RRow(
        "2.13", "7241", "Råvaror och förnödenheter", _CURRENT_ASSETS, ((1410, 1429),)
    ),
    INK2RRow(
        "2.14", "7242", "Varor under tillverkning", _CURRENT_ASSETS, ((1440, 1449),)
    ),
    INK2RRow(
        "2.15",
        "7243",
    ...
    INK2RRow(
        "2.16", "7244", "Övriga lagertillgångar", _CURRENT_ASSETS, ((1490, 1499),)
    ),
    INK2RRow(
        "2.17",
        "7245",
    ...
    INK2RRow(
        "2.18", "7246", "Förskott till leverantörer", _CURRENT_ASSETS, ((1480, 1489),)
    ),
    INK2RRow(
        "2.19", "7251", "Kundfordringar", _CURRENT_ASSETS, ((1510, 1559), (1580, 1589))
    ),
    INK2RRow(
        "2.20",
        "7252",
        "Fordringar hos koncern-, intresse- och gemensamt styrda företag",
        _CURRENT_ASSETS,
        ((1560, 1572), (1574, 1579), (1660, 1672), (1674, 1679)),
    ),
    INK2RRow(
        "2.21",
        "7261",
        ...
        _CURRENT_ASSETS,
        ((1573, 1573), (1610, 1619), (1630, 1659), (1673, 1673), (1680, 1699)),
    ),
    INK2RRow(
        "2.22",
        "7262",
    ...
    INK2RRow(
        "2.24", "7270", "Andelar i koncernföretag", _CURRENT_ASSETS, ((1860, 1869),)
    ),
    INK2RRow(
        "2.25",
        "7271",
    ...
    INK2RRow(
        "2.29", "7321", "Periodiseringsfonder", _UNTAXED, ((2110, 2139),), "credit"
    ),
    INK2RRow(
        "2.30",
        "7322",
    ...
    INK2RRow(
        "2.36", "7351", "Checkräkningskredit", _LIABILITIES, ((2330, 2339),), "credit"
    ),
    INK2RRow(
        "2.37",
        "7352",
    ...
    INK2RRow(
        "2.40", "7360", "Checkräkningskredit", _LIABILITIES, ((2480, 2489),), "credit"
    ),
    INK2RRow(
        "2.41",
        "7361",
    ...
    INK2RRow(
        "2.42", "7362", "Förskott från kunder", _LIABILITIES, ((2420, 2429),), "credit"
    ),
    INK2RRow(
        "2.43",
        "7363",
    ...
    INK2RRow(
        "2.45", "7365", "Leverantörsskulder", _LIABILITIES, ((2440, 2449),), "credit"
    ),
    ...
    INK2RRow(
        ...
        "Skulder till koncern-, intresse- och gemensamt styrda företag",
        _LIABILITIES,
        ((2460, 2472), (2474, 2479), (2860, 2872), (2874, 2879)),
        "credit",
    ),
    INK2RRow(
        "2.48",
    ...
    INK2RRow(
        ...
        _INCOME_STATEMENT,
        ((4900, 4909), (4930, 4959), (4970, 4979), (4990, 4999)),
        "credit",
        negative_field="7510",
        sign="+",
    ),
    ...
    INK2RRow(
        ...
        _INCOME_STATEMENT,
        ((8113, 8113), (8118, 8118), (8123, 8123), (8133, 8133)),
        "credit",
        negative_field="7530",
        sign="+",
    ),
...
CREDIT_FIELDS = frozenset(row.field for row in INK2R_ROWS if row.kind == "credit")
...
EXPENSE_FIELDS: Tuple[str, ...] = tuple(
    dict.fromkeys(
        [
            row.field
            for row in INK2R_ROWS
            if row.section == _INCOME_STATEMENT and row.kind == "cost"
        ]
        + [
            row.negative_field
            for row in INK2R_ROWS
...
BALANCE_EQUITY_LIABILITY_FIELDS: Tuple[str, ...] = tuple(
    row.field for row in INK2R_ROWS if row.section in (_EQUITY, _UNTAXED, _LIABILITIES)
)
...
def field_labels() -> Dict[str, str]:
    ...
    labels: Dict[str, str] = {code: row.label for code, row in ROWS_BY_FIELD.items()}
...
def account_ranges_label(row: INK2RRow) -> str:
    ...
    parts = [
        str(start) if start == end else f"{start}-{end}" for start, end in row.accounts
    ]
    return ", ".join(parts)

...
async def get_agent_instruction_entrypoint():
    ...
    return {
        ...
        "idempotency_contract": {
            ...
            "value_format": "UUID generated by the agent before the call",
            "required_on": [
            ...
            "retry_rule": (
                "Retry a lost, timed-out or interrupted call with the same key "
                "and the same body. A replayed response carries the header "
                "Idempotent-Replay: true and means the voucher is already "
                "posted; treat it as a receipt, not an error."
            ),
            "one_key_per_event": (
                "A new business event always gets a new key. Never reuse a key "
                "for different content."
            ),
            "conflicts": {
                "422": (
                    "idempotency_key_reuse - same key with a different body; "
                    "use a new key for a new event."
                ),
                "409": (
                    "request_in_flight - a call with this key is running; "
                    "retry with the same key."
                ),
                "400": "invalid_idempotency_key - the value is not a UUID.",
            },
            "transition": (
                "A call without the header still succeeds and is logged as "
                "idempotency_key_missing; the header becomes mandatory in a "
                "later version."
            ),
        },
        "workflow_endpoints": {
        ...
        "guardrails": [
            ...
            (
                "Send an Idempotency-Key on every voucher post, and repeat the "
                "same key on a retry; a response with Idempotent-Replay: true "
                "means the voucher is already posted."
            ),
        ...
        "unsupported_features": [
            ...
            "Durable idempotency covers /api/v1/agent/vouchers and /api/v1/vouchers/{voucher_id}/correct; other write endpoints do not replay a stored response yet.",
